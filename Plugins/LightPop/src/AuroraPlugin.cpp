...
void initPlugin(){
    ...
    // quantizes the layout into frameslices
...
{
    // pick random panel light source is centred on
...
void renderPanel(Panel *panel, int *returnR, int *returnG, int *returnB)
{
    ...
    for (i = 0; i < nSources; i++) {
        if (panel->shape->getCentroid().x == sources[i].x && panel->shape->getCentroid().y == sources[i].y) {
