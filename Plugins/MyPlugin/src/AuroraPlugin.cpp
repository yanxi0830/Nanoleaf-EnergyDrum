...
void addSource(float colour, float intensity, float speed)
{
    ...
    count++;
    if (count >= SKIP_COUNT) {
        r = (int)(drand48() * layoutData->nPanels);
        count = 0;
    }
...
{
    // propogate the oldest source
    sources[0].x += sources[0].speed * ADJACENT_PANEL_DISTANCE * 0.05;
    sources[0].y += sources[0].speed * ADJACENT_PANEL_DISTANCE * 0.05;
    float d = distance(0.0, 0.0, sources[0].x, sources[0].y);
